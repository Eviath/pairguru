...
require "title_brackets_validator"  # lib/validators/title_brackets_validator.rb
...
describe TitleBracketsValidator do
  ...
  context "with closed brackets" do
    let(:title) { "The Fellowship of the Ring [Test (Something)]" }
    it_behaves_like "has valid title"
  end

  # validation with not empty brackets
  context "with not empty brackets" do
    let(:title) { "The Fellowship of the Ring (Something)" }
    it_behaves_like "is not empty"
  end
end
